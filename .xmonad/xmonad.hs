...
myBrowser1 = "/opt/firefox/firefox-bin"
myBrowser2 = "google-chrome-stable"
myEditor = "/opt/sublime_text_3/sublime_text"
myIDE = "pycharm"

-- The command to lock the screen or show the screensaver.
myScreensaver = "i3lock -d -c 000000"

-- The command to take a selective screenshot, where you select
-- what you'd like to capture on the screen.
mySelectScreenshot = "shutter --section"
...
--
myWorkspaces = ["1: zabbix","2: slack","3: term"] ++ map show [5..9]
...
myManageHook = composeAll
    [
      className =? "Google-chrome"                --> doShift "1: zabbix"
    , resource  =? "desktop_window"               --> doIgnore
    , className =? "VirtualBox"                   --> doShift "1: zabbix"
    , className =? "Slack"                        --> doShift "2: slack"
    , className =? "Firefox Developer Edition"    --> doShift "3: term"
    , className =? "Terminator"                   --> doShift "3: term"
    , className =? "Sublime_text"                 --> doShift "3: term"
    , className =? "jetbrains-pycharm"            --> doShift "3: term"
    , className =? "stalonetray"                  --> doCenterFloat
...
myFont      = "xft:Zekton:size=8:bold:antialias=true"
myBigFont   = "xft:Zekton:size=8:bold:antialias=true"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- Start the 1st browser - mod + Shift + G for Google
  , ((modMask .|. controlMask, xK_g),
     spawn myBrowser2)

  -- Start the 1st browser - mod + Shift + F for Google
  , ((modMask .|. controlMask, xK_f),
     spawn myBrowser1)

  -- Start the the editorr - mod + Shift + E
  , ((modMask .|. controlMask, xK_e),
     spawn myEditor)

  -- Start the the PyCharm - mod + Shift + I
  ...
  -- Move focus to the next window with Super+Tab.
  , ((modMask, xK_Tab),
     windows W.focusDown)
  ...
  -- conflicts with Mod+Enter for terminal
  -- , ((modMask, xK_Return),
  ...
  -- Toggle the status bar gap.NNNNNN
  ...
  [((m .|. modMask, key), screenWorkspace sc >>= flip whenJust (windows . f)) -- Replace 'mod1Mask' with your mod key of choice.
      | (key, sc) <- zip [xK_w, xK_e, xK_r] [3,1,2] -- was [0..] *** change to match your screen order ***
  ...
  -- Conflicts with cycling through the windows
